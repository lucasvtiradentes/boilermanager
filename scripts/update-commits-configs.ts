...
class CommitsConfigs {
  ...
  private updateJsonContent(file: string, cb: (content: any) => Promise<any>) {
    if (!existsSync(file)) {
  ...
  setupUsedTypes(usedCommits: ValidTypes[]) {
    this.usedCommits = usedCommits;
  }

  getCommitzenTypesArray() {
    const validCommits = [...this.usedCommits].filter((type) => this.allgitmojiArr.find((gitMojiItem: any) => gitMojiItem.type === type) !== null);
    const commitzenArr = [...validCommits].map((type) => {
      const gitmojiItem = this.allgitmojiArr.find((gitMojiItem: any) => gitMojiItem.type === type);
      return {
